...
Pxy = nonCrossEdge(Px,Py);
%% linear optimization to get Pxy
% A1 = zeros(n,m*n);
% for ii=1:n
%     A1(ii,(ii-1)*m+1:ii*m) = Px';
% end
% blc1 = Py;
% buc1 = Py;
% 
...
% 
% param = [];
% param.MSK_IPAR_INTPNT_BASIS = 'MSK_BI_NO_ERROR';
% param.MSK_IPAR_BI_IGNORE_MAX_ITER = 'MSK_ON';
% param.MSK_IPAR_LOG = 0;
% param.MSK_DPAR_INTPNT_NL_TOL_PFEAS = 1.0e-12;
% param.MSK_DPAR_INTPNT_NL_TOL_DFEAS = 1.0e-12;
% param.MSK_DPAR_INTPNT_NL_TOL_REL_GAP = 1.0e-12;
% 
% [res] = msklpopt(c,A,blc,buc,blx,bux,param);
% yy = res.sol.itr.xx;
% yy(abs(yy)<=1e-6)=0;%-4
% Pxy = reshape(yy,m,n);
Dav = trace(Dxy*Pxy');
...
fprintf('minDistortionEmr Iteration: %d\t Distortion: %f\t Sum of Py: %f\n',iters,Dav,sum(Py));
